...
export default async function HomePage({ searchParams }: PageProps) {
	const { exampleString, redirect: redirectIfNotAuthed } =
		await exampleSearchParams(searchParams);
	...
	return (
		<React.Fragment>
			<div>
				...
				<p className="mt-2">
					Below is returned data from the authed procedure. It only returns said
					data if you (the user) are authenticated. You may also pass a search
					param of{" "}
					<span className="border bg-white font-mono text-sm">
						redirect=false
					</span>{" "}
					to see the unauthenticated API error.
				</p>
			</div>
